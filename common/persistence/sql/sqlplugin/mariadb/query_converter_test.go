...
func TestQueryConverter_GetCoalesceCloseTimeExpr(t *testing.T) {
	...
	r.Equal("close_time_or_max", sqlparser.String(expr))
}
...
func TestQueryConverter_BuildSelectStmt(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			pageSize: 10,
			stmt: fmt.Sprintf(
				"SELECT %s FROM executions_visibility ev LEFT JOIN custom_search_attributes USING (namespace_id, run_id) LEFT JOIN chasm_search_attributes USING (namespace_id, run_id) ORDER BY close_time_or_max DESC, start_time DESC, run_id LIMIT ?",
		...
		},
		{
			...
			pageSize: 20,
			stmt: fmt.Sprintf(
				"SELECT %s FROM executions_visibility ev LEFT JOIN custom_search_attributes USING (namespace_id, run_id) LEFT JOIN chasm_search_attributes USING (namespace_id, run_id) WHERE Keyword01 = 'foo' ORDER BY close_time_or_max DESC, start_time DESC, run_id LIMIT ?",
				strings.Join(dbFields, ", "),
			),
			queryArgs: []any{20},
		},
		{
			...
			stmt: fmt.Sprintf(
				"SELECT %s FROM executions_visibility ev LEFT JOIN custom_search_attributes USING (namespace_id, run_id) LEFT JOIN chasm_search_attributes USING (namespace_id, run_id) WHERE Keyword01 = 'foo' AND ((close_time_or_max = ? AND start_time = ? AND run_id > ?) OR (close_time_or_max = ? AND start_time < ?) OR close_time_or_max < ?) ORDER BY close_time_or_max DESC, start_time DESC, run_id LIMIT ?",
				strings.Join(dbFields, ", "),
			),
			queryArgs: []any{
				closeTime,
