...
var (
	errVisInterpolateParamsNotSupported = errors.New("interpolateParams is not supported for mariadb visibility stores")
...
func buildDSNAttrs(dbKind sqlplugin.DbKind, cfg *config.SQL) (map[string]string, error) {
	...
	// Enable interpolation by default unless this is a mariadb visibility store
...
func sanitizeAttr(inkey string, invalue string) (string, string) {
	...
	case isolationLevelAttrName, isolationLevelAttrNameLegacy:
		if value[0] != '\'' { // mariadb sys variable values must be enclosed in single quotes
			value = "'" + value + "'"
		}
		return key, value
	default:
