...
import (
	...
	"go.temporal.io/server/common/persistence/sql/sqlplugin/mariadb/session"
	"go.temporal.io/server/common/resolver"
)

const (
	// PluginName is the name of the plugin
	PluginName = "mariadb10"
)
...
func (p *plugin) createDBConnection(
	...
	mariadbSession, err := session.NewSession(dbKind, cfg, resolver)
	if err != nil {
		return nil, err
	}
	return mariadbSession.DB, nil
}
